package com.camera.internal.manager.impl;
...
public class Camera1Manager extends BaseCameraManager<Integer, SurfaceHolder.Callback> {
    ...
    @Override
    public void openCamera(File outputPath, final Integer cameraId,
    ...
    @Override
    public void takePhoto(CameraPhotoListener cameraPhotoListener) {
                ...
                public void run() {
                    ...
                    camera.takePicture(null, null, new Camera.PictureCallback() {
                        ...
                        public void onPictureTaken(byte[] bytes, Camera camera) {
                            Camera1Manager.this.onPictureTaken(bytes);
                        }
                    });
                }
            });
    ...
    @Override
    public void startVideoRecord(CameraVideoListener cameraVideoListener) {
    ...
    protected void prepareCameraOutputs() {
        try {
            ...
            windowSize = new Size(ScreenUtils.getScreenWidth(), ScreenUtils.getScreenHeight());

            videoSize = CameraHelper.getSizeWithClosestRatio(
                    (videoSizes == null || videoSizes.isEmpty()) ? previewSizes : videoSizes,
                    camcorderProfile.videoFrameWidth, camcorderProfile.videoFrameHeight);

            photoSize = CameraHelper.getOptimalPictureSize(
                    (pictureSizes == null || pictureSizes.isEmpty()) ? previewSizes : pictureSizes, windowSize.getWidth(), windowSize.getHeight(),
            ...
            if (configurationProvider.getMediaAction() == Configuration.MEDIA_ACTION_PHOTO
                ...
                previewSize = CameraHelper.getOptimalPreviewSize(previewSizes, windowSize.getWidth(), windowSize.getHeight());
            } else {
                previewSize = CameraHelper.getOptimalPreviewSize(previewSizes, windowSize.getWidth(), windowSize.getHeight());
            }
        } catch (Exception e) {
...
//                parameters.setRecordingHint(true);
    ...
    protected void onPictureTaken(final byte[] bytes) {
        ...
        if (configurationProvider.getCoverType() == Configuration.COVERTYPE_RECTANGLE) {
            //接下来获取指定区域内的图片
            //屏幕和预览的缩放比例
            ...
            if (ScreenUtils.isPortrait()) {
                ratio = ScreenUtils.isPortrait() ? (double) photoSize.getWidth() / ScreenUtils.getScreenHeight() : (double) photoSize.getHeight() / ScreenUtils.getScreenHeight();//预览的高度就是屏幕的宽度
                startX = (int) (middleRect.top * ratio);
                startY = (int) (photoSize.getHeight() - middleRect.right * ratio);
                width = (int) ((middleRect.bottom - middleRect.top) * ratio);
                height = (int) ((middleRect.right - middleRect.left) * ratio);
            } else {
                ratio = (double) photoSize.getHeight() / ScreenUtils.getScreenHeight();//预览的高度就是屏幕的宽度
                startX = (int) (middleRect.left * ratio);
                startY = (int) (middleRect.top * ratio);
                width = (int) ((middleRect.right - middleRect.left) * ratio);
                height = (int) ((middleRect.bottom - middleRect.top) * ratio);
            }


            Log.e("image", "left==" + middleRect.left + " top==" + middleRect.top + " right==" + middleRect.right + " bottom==" + middleRect.bottom);
            Log.e("image", "startX==" + startX + " startY==" + startY + " width==" + width + " height==" + height);
            Log.e("image", " photoSize.getHeight()==" + photoSize.getHeight() + " photoSize.getWidth()==" + photoSize.getWidth() + " ScreenUtils.getScreenHeight()==" + ScreenUtils.getScreenHeight() + " ScreenUtils.getScreenWidth()==" + ScreenUtils.getScreenWidth() + " ratio==" + ratio);

            Bitmap clip = ImageUtils.clip(ImageUtils.getBitmap(bytes, 0), startX, startY, width, height, true);
            String imageType = ImageUtils.getImageType(outputPath);
            tempbytes = ConvertUtils.bitmap2Bytes(clip, "PNG".equals(imageType) ? Bitmap.CompressFormat.PNG : Bitmap.CompressFormat.JPEG);
        }
        FileIOUtils.writeFileFromBytesByStream(outputPath, tempbytes, false);
    ...
    private void setfaceDetection(Camera camera) {
