...
import com.camera.CameraFragmentApi;
import com.camera.configuration.Configuration;
import com.camera.configuration.ConfigurationProvider;
import com.camera.configuration.ConfigurationProviderImpl;
import com.camera.internal.controller.CameraController;
import com.camera.internal.controller.impl.Camera1Controller;
import com.camera.internal.controller.impl.Camera2Controller;
import com.camera.internal.controller.view.CameraView;
import com.camera.internal.enums.Camera;
import com.camera.internal.enums.Flash;
import com.camera.internal.enums.MediaAction;
import com.camera.internal.enums.Record;
import com.camera.internal.timer.CountdownTask;
import com.camera.internal.timer.TimerTask;
import com.camera.internal.timer.TimerTaskBase;
import com.camera.internal.ui.model.PhotoQualityOption;
import com.camera.internal.ui.model.VideoQualityOption;
import com.camera.internal.ui.view.AspectFrameLayout;
import com.camera.internal.utils.CameraHelper;
import com.camera.internal.utils.PhotoUtils;
...
import com.camera.widgets.cover.SquareCoverView;
...
public abstract class BaseAnncaFragment<CameraId> extends Fragment implements CameraFragmentApi {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        final CameraView cameraView = new CameraView() {
            ...
            public void updateCameraPreview(Size size, View cameraPreview) {
                ...
                if (configurationProvider.faceDetection()) {
                    takePhoto();
                }
            }
      ...
      /*  if (CameraHelper.hasCamera2(getContext()) && !configurationProvider.faceDetection()) {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        final int defaultOrientation = PhotoUtils.getDeviceDefaultOrientation(getContext());
    ...
    public void openSettingDialog() {
        ...
        layoutParams.width = PhotoUtils.convertDipToPixels(context, 350);
        layoutParams.height = PhotoUtils.convertDipToPixels(context, 350);
    ...
    protected void setCameraPreview(View preview, Size previewSize) {
        ...
        double ratio = previewSize.getHeight() / (double) previewSize.getWidth();
        addCover();//添加覆盖物
    ...
    protected void onStopVideoRecord() {
        if (cameraFragmentControlsListener != null) {
            cameraFragmentControlsListener.allowRecord(false);
        }
        if (cameraFragmentStateListener != null) {
    ...
    public void addCover() {
        int coverType = configurationProvider.getCoverType();
        if (coverType != -1) {
            SuperCoverView superCoverView = new SuperCoverView(getContext(), coverType);
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            previewContainer.addView(superCoverView, layoutParams);
        }
    }

    private void smartScan(double ratio) {
        FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        layoutParams.gravity = Gravity.CENTER;
        final SquareCoverView squareCoverView = new SquareCoverView(getContext());
        previewContainer.addView(squareCoverView, layoutParams);

        previewContainer.post(new Runnable() {
            @Override
            public void run() {
                //扫描线
                AppCompatImageView scanView = new AppCompatImageView(previewContainer.getContext());
                scanView.setBackgroundResource(R.drawable.scan_5);
                RectF scanRectF = squareCoverView.getScanRectF();
                FrameLayout.LayoutParams imageLayoutParams = new AspectFrameLayout.LayoutParams((int) (scanRectF.right - scanRectF.left), AspectFrameLayout.LayoutParams.WRAP_CONTENT);

                imageLayoutParams.leftMargin = (int) scanRectF.left;
                imageLayoutParams.topMargin = (int) scanRectF.top;
                previewContainer.addView(scanView, imageLayoutParams);

                ObjectAnimator translationY = new ObjectAnimator().ofFloat(scanView, "translationY", 0, scanRectF.bottom - scanRectF.top - 30);
                translationY.setDuration(3000);  //设置动画时间
                translationY.setRepeatCount(ValueAnimator.INFINITE);
                // translationY.setRepeatMode(ValueAnimator.INFINITE);
