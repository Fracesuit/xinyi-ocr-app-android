...
import static com.luck.picture.lib.tools.PictureFileUtils.POSTFIX;
...
public class Luban implements Handler.Callback {
    ...
    private File getImageCacheFile(String suffix) {
        String fileName = PictureFileUtils.getFileName(TextUtils.isEmpty(suffix) ? POSTFIX : suffix);
        return PictureFileUtils.createFile(PictureFileUtils.getCompressDir(), fileName);
    }
                    ...
                    public void run() {
                        try {
                            ...
                            File result = Checker.isNeedCompress(mLeastCompressSize, path) ?
                                    new Engine(path, getImageCacheFile(Checker.checkSuffix(path))).compress() :
                                    new File(path);
